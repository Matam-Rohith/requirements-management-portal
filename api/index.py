...
def list_requirements(status: Optional[str] = None, priority: Optional[str] = None, search: Optional[str] = None, user=Depends(get_current_user)):
    items = REQUIREMENTS[:]
...
def create_requirement(body: RequirementCreate, user=Depends(require_admin)):
    req = {"id": f"req-{uuid.uuid4().hex[:6]}", "title": body.title, "description": body.description,
           "priority": body.priority, "status": body.status, "assignee": body.assignee, "tags": body.tags or [],
           "created_by": user["username"], "created_at": datetime.utcnow().isoformat(), "updated_at": datetime.utcnow().isoformat()}
...
def delete_requirement(req_id: str, user=Depends(require_admin)):
    ...
    if len(REQUIREMENTS) == before: raise HTTPException(status_code=404, detail="Not found")
