...
export default function DashboardPage({ token }: Props) {
  ...
  return (
    <div className='space-y-6'>
      <div><h1 className='text-xl font-bold'>Dashboard</h1><p className='text-sm text-muted-foreground'>Overview of all requirements</p></div>
      <div className='grid grid-cols-2 md:grid-cols-4 gap-4'>
        ...
        <StatCard title='Completion'  value={`${stats.completion_rate}%`}           icon={TrendingUp}    color='#8b5cf6' sub='rate' index={3} />
      </div>
      {(stats.by_priority['Critical'] || 0) > 0 && (
        <div className='flex items-center gap-2 px-4 py-3 rounded-lg bg-red-50 border border-red-200 text-red-700 text-sm'>
          <AlertTriangle className='w-4 h-4 flex-shrink-0' />
          <span><strong>{stats.by_priority['Critical']}</strong> critical requirements need immediate attention.</span>
        </div>
      )}
