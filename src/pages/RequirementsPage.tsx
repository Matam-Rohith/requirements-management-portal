...
export default function RequirementsPage({ token, isAdmin }: Props) {
  ...
  const handleSubmit = async (data: Partial<Requirement>) => {
    setSaving(true)
    try {
      if (editTarget?.id) await apiFetch(`/requirements/${editTarget.id}`, { method: 'PUT', body: JSON.stringify(data) }, token)
      else await apiFetch('/requirements', { method: 'POST', body: JSON.stringify(data) }, token)
      closeForm(); load()
  ...
  return (
    <div className='space-y-4'>
      <div className='flex items-center justify-between'>
        <div><h1 className='text-xl font-bold'>Requirements</h1><p className='text-sm text-muted-foreground'>{items.length} items</p></div>
      ...
      <div className='flex flex-wrap gap-2'>
        ...
        <select value={filterStatus} onChange={e => setFS(e.target.value)}
          ...
          <option value=''>All Status</option>{STATUS_ORDER.map(s => <option key={s}>{s}</option>)}
        </select>
        <select value={filterPriority} onChange={e => setFP(e.target.value)}
          className='px-3 py-2 text-sm border border-input rounded-md bg-background focus:outline-none cursor-pointer'>
          <option value=''>All Priority</option>{PRIORITY_ORDER.map(p => <option key={p}>{p}</option>)}
        </select>
      </div>
      {loading ? <div className='text-center py-12 text-muted-foreground text-sm'>Loading...</div>
        : items.length === 0 ? <div className='text-center py-12 text-muted-foreground text-sm'>No requirements found.</div>
        : <div className='grid gap-3'>{items.map((r, i) => <RequirementCard key={r.id} req={r} isAdmin={isAdmin} onEdit={openEdit} onDelete={handleDelete} index={i} />)}</div>
      }
      <Dialog open={showForm} onOpenChange={open => !open && closeForm()}>
        {showForm && editTarget !== null && <RequirementForm initial={editTarget} onSubmit={handleSubmit} onClose={closeForm} loading={saving} />}
      </Dialog>
    </div>
  )
}
